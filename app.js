...
//Elements 
  ...
  const defenders = Array.from(cells).slice(defenderStartIndex, defenderStartIndex + width)
    
  // ? keeper
  const keeperRowIndex = 7
  const keeperSpace = 2
  const keeper = Array.from(cells).slice((width / 5 ) * keeperRowIndex, (width / 5 ) *  (keeperRowIndex + 1))
    
  // ? defendingLine
  const defendingLineIndex1 = 4
  const defendingLineSpace = 3
  const defendingLine1 = Array.from(cells).slice(width * defendingLineIndex1, width * (defendingLineIndex1 + 1))
    
  const defendingLineIndex2 = 5
  const defendingLineSpace2 = 2
  const defendingLine2 = Array.from(cells).slice(width * defendingLineIndex2, width * (defendingLineIndex2 + 1))
  
  let counter = 0
    
  function run(){
    counter ++
    cells.forEach(cell =>cell.classList.remove('defender' , 'keeper', 'defendingLine', 'defendingLine2'))
    ...
    defendingLine1.forEach((defendingLine, index) => {
      if (index % defendingLineSpace === defendingLineSpace - 1 - counter % defendingLineSpace){
